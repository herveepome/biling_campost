...
//générer la facture
    ...
    public function generate_bill_file() {
        ...
        if ($this->input->post()) {
            ...
            if (!empty($bill_file) && $bill_file != null) {
                ...
                $error = "Ce fichier ne peut pas encore être généré car le fichier de facturation  correspondant à ces critères n'a pas encore été chargé. Veuillez le charger et réessayer";
                $this->session->message = $error;
    ...
    public function generate_facture($test, $name_file, $name, $data, $type, $file_text_name, $facturation_date, $file_name, $customer_id, $period, $headers, $file, $newfilelisting, $newfilefact, $path, $file_type, $billing_id = null) {
        ...
        if (!empty($this->state_model->getALL(array("period" => $period, "type" => "LF", "customerID" => $customer_id[0]->id))) || !empty($this->state_model->getALL(array("period" => $period, "type" => "F", "customerID" => $customer_id[0]->id)))
        ) {
            ...
            if ($data["facture"] != null && !empty($data["facture"]))
                $this->facture_file($data["facture"], $name["facture"], $bill_number, "PERIODE: " . $this->monthinFrench(date('F', mktime(0, 0, 0, substr($period, 2, 2)))) . " " . substr($period, 4), $customer_id[0]);
